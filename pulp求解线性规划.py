...
def main():
    ...
    线性规划问题字典 = {
        ...
        '假设EXP溢出': pulp.LpProblem(sense=pulp.LpMaximize),
    ...
    线性规划问题字典['假设无溢出'] += 平均每日产出EXP * 钱书需求比下限 <= 平均每日产出龙门币
    线性规划问题字典['假设无溢出'] += 平均每日产出龙门币 <= 平均每日产出EXP * 钱书需求比上限

    线性规划问题字典['假设EXP溢出'] += 基建综合产出_假设EXP溢出
    线性规划问题字典['假设EXP溢出'] += 平均每日产出EXP * 钱书需求比下限 >= 平均每日产出龙门币

    线性规划问题字典['假设龙门币溢出'] += 基建综合产出_假设龙门币溢出
    线性规划问题字典['假设龙门币溢出'] += 平均每日产出EXP * 钱书需求比上限 <= 平均每日产出龙门币
    ...
    print(f'钱书产出比：{最大结果['平均每日产出龙门币'] / 最大结果['平均每日产出EXP']:.4f}')
    print(f'平均每日产出EXP：{最大结果['平均每日产出EXP']:.4f}')
    ...
    for 排班表序号 in range(排班表数量):
        ...
        无人机加速中级作战记录比例 = divide(最大结果['无人机加速中级作战记录比例乘运行时长比例'][排班表序号], 运行时长比例)
