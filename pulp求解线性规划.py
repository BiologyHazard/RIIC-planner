...
def main():
    ...
    无人机加速中级作战记录比例乘运行时长比例变量 = [
        pulp.LpVariable(f'无人机加速中级作战记录比例乘运行时长比例_{排班表['排班表名称']}')
        for 排班表 in 候选排班表
    ]
    无人机加速赤金比例乘运行时长比例变量 = [
    ...
    基建综合产出变量 = pulp.LpVariable('基建综合产出')

    # 中间变量
    平均每日产出EXP = pulp.lpSum(
        候选排班表[排班表序号]['每日产出']['EXP'] * 运行时长比例变量[排班表序号]
        + (无人机加速中级作战记录获得EXP
           * 候选排班表[排班表序号]['每日产出']['无人机']
           * 无人机加速中级作战记录比例乘运行时长比例变量[排班表序号])
    ...
    基建综合产出_假设EXP溢出 = (
        平均每日产出龙门币 / 钱书需求比下限 * EXP价值
        + (平均每日产出EXP - 平均每日产出龙门币 / 钱书需求比下限) * EXP价值 * 溢出折算系数
        + 平均每日产出龙门币 * 龙门币价值
        + 平均每日产出信用 * 信用价值
        + 平均每日产出公开招募刷新 * 公开招募刷新价值
    )
    基建综合产出_假设龙门币溢出 = (
        平均每日产出EXP * EXP价值
        + (平均每日产出EXP * 钱书需求比上限) * 龙门币价值
        + (平均每日产出龙门币 - 平均每日产出EXP * 钱书需求比上限) * 龙门币价值 * 溢出折算系数
        + 平均每日产出信用 * 信用价值
        + 平均每日产出公开招募刷新 * 公开招募刷新价值
    )
    ...
    无人机加速中级作战记录约束条件 = [
        候选排班表[排班表序号]['每日产出']['无人机'] * 无人机加速中级作战记录比例乘运行时长比例变量[排班表序号]
        >= 候选排班表[排班表序号]['至少分配无人机用于加速']['中级作战记录'] * 运行时长比例变量[排班表序号]
        for 排班表序号 in range(排班表数量)
    ]
    无人机加速赤金约束条件 = [
    ...
    基建综合产出约束条件 = [
        基建综合产出变量 <= 基建综合产出_假设无溢出,
        基建综合产出变量 <= 基建综合产出_假设EXP溢出,
        基建综合产出变量 <= 基建综合产出_假设龙门币溢出,
    ]
    ...
    变量 = {
        # 未知量
        '运行时长比例': 运行时长比例变量,
        '无人机加速中级作战记录比例乘运行时长比例': 无人机加速中级作战记录比例乘运行时长比例变量,
    ...
    # 定义线性规划问题
    线性规划问题 = pulp.LpProblem(sense=pulp.LpMaximize)
    线性规划问题 += 基建综合产出变量
    for 约束条件 in 约束条件列表:
        线性规划问题 += 约束条件
    ...
    if 求解状态 != pulp.LpStatusOptimal:
        raise ValueError(f'求解失败，状态为{pulp.LpStatus[求解状态]}')
    求解结果 = 递归获取变量的值(变量)

    # 输出结果
    print(f'基建综合产出：{求解结果['基建综合产出']:.4f}')
    print(f'钱书产出比：{求解结果['平均每日产出龙门币'] / 求解结果['平均每日产出EXP']:.4f}')
    print(f'平均每日产出EXP：{求解结果['平均每日产出EXP']:.4f}')
    print(f'平均每日产出赤金：{求解结果['平均每日产出赤金']:.4f}')
    print(f'平均每日产出龙门币：{求解结果['平均每日产出龙门币']:.4f}')
    print(f'平均每日产出源石碎片：{求解结果['平均每日产出源石碎片']:.4f}')
    print(f'平均每日产出合成玉：{求解结果['平均每日产出合成玉']:.4f}')
    print(f'平均每日产出信用：{求解结果['平均每日产出信用']:.4f}')
    print(f'平均每日产出公开招募刷新：{求解结果['平均每日产出公开招募刷新']:.4f}')

    for 排班表序号 in range(排班表数量):
        运行时长比例 = 求解结果['运行时长比例'][排班表序号]
        无人机加速中级作战记录比例 = divide(求解结果['无人机加速中级作战记录比例乘运行时长比例'][排班表序号], 运行时长比例)
        无人机加速赤金比例 = divide(求解结果['无人机加速赤金比例乘运行时长比例'][排班表序号], 运行时长比例)
        无人机加速龙门币比例 = [
            divide(求解结果['无人机加速龙门币比例乘运行时长比例'][排班表序号][贸易站序号], 运行时长比例)
            for 贸易站序号 in range(候选排班表[排班表序号]['贸易站数量'])
        ]
        无人机加速源石碎片比例 = divide(求解结果['无人机加速源石碎片比例乘运行时长比例'][排班表序号], 运行时长比例)
        无人机加速合成玉比例 = divide(求解结果['无人机加速合成玉比例乘运行时长比例'][排班表序号], 运行时长比例)
        无人机加速中级作战记录数量 = 无人机加速中级作战记录比例 * 候选排班表[排班表序号]['每日产出']['无人机']
        ...
        每日产出EXP = (
            ...
            + 无人机加速中级作战记录获得EXP * 无人机加速中级作战记录数量
        )
        每日产出赤金 = (
            候选排班表[排班表序号]['每日产出']['赤金']
        ...
        print(f'无人机加速中级作战记录：{无人机加速中级作战记录数量:.4f} ({无人机加速中级作战记录比例:.2%})')
