...
def main():
    ...
    def 无人机加速作战记录数量(变量, 排班表序号):
        return 候选排班表[排班表序号]['每日产出']['无人机'] * 无人机加速作战记录比例(变量, 排班表序号)
    ...
    def 作战记录产出函数(变量, 排班表序号):
        return (
            ...
            + 无人机加速作战记录获得作战记录 * 无人机加速作战记录数量(变量, 排班表序号)
        )
    ...
    约束条件.append(optimize.NonlinearConstraint(合成玉综合产出函数, lb=每日需求合成玉 - 1e-2, ub=每日需求合成玉 + 1e-2))  # 非线性约束条件如果要求严格相等可能导致无解
    运行时长约束向量 = [0] * 变量个数
    for 排班表序号 in range(排班表数量):
    ...
    for 排班表序号, 排班表 in enumerate(候选排班表):
        ...
        print(f'无人机加速作战记录：{无人机加速作战记录数量(结果, 排班表序号):.4f} ({无人机加速作战记录比例(结果, 排班表序号):.2%})')
