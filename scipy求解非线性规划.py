...
def main():
    ...
    def 基建综合产出(变量):
        ...
        作战记录符合钱书比的部分 = min(作战记录综合产出, 龙门币综合产出 / 钱书需求比下限)
        龙门币符合钱书比的部分 = min(龙门币综合产出, 作战记录综合产出 * 钱书需求比上限)
        溢出的作战记录 = 作战记录综合产出 - 作战记录符合钱书比的部分
        溢出的龙门币 = 龙门币综合产出 - 龙门币符合钱书比的部分
        return (
            作战记录符合钱书比的部分 * 作战记录价值
            + 龙门币符合钱书比的部分 * 龙门币价值
    ...
    约束条件.append(optimize.NonlinearConstraint(合成玉综合产出函数, lb=每日需求合成玉 - 0.01, ub=每日需求合成玉 + 0.01))  # 非线性约束条件如果卡死可能导致无解
    运行时长约束向量 = [0] * 变量个数
    for 排班表序号 in range(排班表数量):
    ...
    print(f'钱书产出比：{龙门币综合产出函数(结果) / 作战记录综合产出函数(结果):.4f}')
    ...
    for 排班表序号, 排班表 in enumerate(候选排班表):
        ...
        print(f'无人机加速源石碎片比例：{无人机加速源石碎片比例(结果, 排班表序号):.2%}')
        print(f'无人机加速合成玉比例：{无人机加速合成玉比例(结果, 排班表序号):.2%}')
