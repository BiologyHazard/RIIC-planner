...
每秒基础工时生产赤金 = 1 / (72 * 60)
每秒基础工时生产源石碎片 = 1 / 3600
# 每秒基础工时使用固源岩生产源石碎片消耗龙门币 = 1600 / 3600
# 每秒基础工时使用装置生产源石碎片消耗龙门币 = 1000 / 3600
...
# 无人机加速使用固源岩生产源石碎片消耗龙门币 = 无人机相当于秒基础工时 * 每秒基础工时使用固源岩生产源石碎片消耗龙门币
# 无人机加速使用装置生产源石碎片消耗龙门币 = 无人机相当于秒基础工时 * 每秒基础工时使用装置生产源石碎片消耗龙门币
...
def main():
    ...
    def 作战记录产出函数(变量, 排班表序号):
        return (
            ...
            + 候选排班表[排班表序号]['每日产出']['无人机'] * 无人机加速作战记录获得作战记录 * 无人机加速作战记录比例(变量, 排班表序号)
        )
    ...
    def 赤金产出函数(变量, 排班表序号):
        return (
            候选排班表[排班表序号]['每日产出']['赤金']
            + 候选排班表[排班表序号]['每日产出']['无人机'] * 无人机加速赤金获得赤金 * 无人机加速赤金比例(变量, 排班表序号)
            - sum(
    ...
    def 龙门币产出函数(变量, 排班表序号):
        return (
            ...
            - 候选排班表[排班表序号]['每日产出']['无人机'] * 无人机加速源石碎片消耗龙门币 * 无人机加速源石碎片比例(变量, 排班表序号)
        )
    ...
    def 源石碎片产出函数(变量, 排班表序号):
        return (
            ...
            + 候选排班表[排班表序号]['每日产出']['无人机'] * 无人机加速源石碎片获得源石碎片 * 无人机加速源石碎片比例(变量, 排班表序号)
            - 候选排班表[排班表序号]['每日产出']['无人机'] * 无人机加速合成玉消耗源石碎片 * 无人机加速合成玉比例(变量, 排班表序号)
        )
    ...
    def 合成玉产出函数(变量, 排班表序号):
        return (
            ...
            + 候选排班表[排班表序号]['每日产出']['无人机'] * 无人机加速合成玉获得合成玉 * 无人机加速合成玉比例(变量, 排班表序号)
        )
    ...
    def 基建综合产出(变量):
        ...
        符合钱书比的部分_以作战记录计 = min(作战记录综合产出, 龙门币综合产出 / 钱书需求比)
        溢出的作战记录 = 作战记录综合产出 - 符合钱书比的部分_以作战记录计
        溢出的龙门币 = 龙门币综合产出 - 符合钱书比的部分_以作战记录计 * 钱书需求比
        return (
            符合钱书比的部分_以作战记录计 * 作战记录价值
            + 符合钱书比的部分_以作战记录计 * 钱书需求比 * 龙门币价值
            + (溢出的作战记录 * 作战记录价值 + 溢出的龙门币 * 龙门币价值) * 溢出折算系数
            + 信用综合产出 * 信用价值
            + 公开招募刷新综合产出 * 公开招募刷新价值
        )
    ...
    约束条件 = []
    约束条件.append(optimize.NonlinearConstraint(赤金综合产出函数, lb=-基建外每日获取赤金, ub=math.inf))
    约束条件.append(optimize.NonlinearConstraint(源石碎片综合产出函数, lb=0, ub=math.inf))
    约束条件.append(optimize.NonlinearConstraint(合成玉综合产出函数, lb=每日需求合成玉 + 0.01, ub=每日需求合成玉 + 0.01))
    运行时长约束向量 = [0] * 变量个数
    for 排班表序号 in range(排班表数量):
    ...
    result: optimize.OptimizeResult = optimize.minimize(目标函数, 变量初值, bounds=变量范围, constraints=约束条件, options={'maxiter': 512})
    print(result)
    ...
    print(f'源石碎片综合产出：{源石碎片综合产出函数(结果):.4f}')
    print(f'合成玉综合产出：{合成玉综合产出函数(结果):.4f}')
    ...
    for 排班表序号, 排班表 in []:  # enumerate(候选排班表):
        ...
        for 贸易站序号, 贸易站 in enumerate(排班表['贸易站列表']):
            print(f'    {贸易站.贸易站名称}：{无人机加速龙门币比例(结果, 排班表序号, 贸易站序号):.2%}')
